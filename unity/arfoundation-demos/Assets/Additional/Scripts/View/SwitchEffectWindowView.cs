...
{
    [SerializeField] private Toggle _switchEffectModeToggle;
    // Start is called before the first frame update
     private Subject<bool> _OnToggleClicked = new Subject<bool>();
    public IObservable<bool> OnToggleClicked => _OnToggleClicked;
    private void Awake()
    {
        _switchEffectModeToggle.OnValueChangedAsObservable().Subscribe(_OnToggleClicked.OnNext);
    }
    
    //C# 7.0 以降では、式形式で値の変更が可能．
    public void ActivateToggle(bool isOn) => _switchEffectModeToggle.isOn = isOn;
